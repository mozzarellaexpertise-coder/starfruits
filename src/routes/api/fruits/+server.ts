...
export const GET: RequestHandler = async () => {
  ...
  return new Response(JSON.stringify(fruits), { status: 200, headers });
};
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  return new Response(JSON.stringify(newFruit), { status: 201, headers });
};
...
export const PUT: RequestHandler = async ({ request }) => {
  ...
  return new Response(JSON.stringify(updatedFruit), { status: 200, headers });
};
